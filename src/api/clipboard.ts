...
function openDB():Promise<IDBDatabase>{return new Promise((resolve,reject)=>{const req=indexedDB.open(DB_NAME,DB_VERSION);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(STORE_CLIPS)){const s=db.createObjectStore(STORE_CLIPS,{keyPath:'id',autoIncrement:true});s.createIndex('hash','hash',{unique:true});s.createIndex('createdAt','createdAt');s.createIndex('type','type');s.createIndex('pinned','pinned')}if(!db.objectStoreNames.contains(STORE_MEMOS)){const m=db.createObjectStore(STORE_MEMOS,{keyPath:'id',autoIncrement:true});m.createIndex('createdAt','createdAt')}if(!db.objectStoreNames.contains(STORE_SETTINGS)){db.createObjectStore(STORE_SETTINGS)}};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
...
export async function getHistory(limit=200):Promise<ClipEntry[]>{const db=await openDB();return new Promise((resolve)=>{const results:ClipEntry[]=[];db.transaction(STORE_CLIPS,'readonly').objectStore(STORE_CLIPS).index('createdAt').openCursor(null,'prev').onsuccess=(e:any)=>{const cursor=e.target.result;if(cursor&&results.length<limit){results.push(cursor.value);cursor.continue()}else{results.sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)||b.createdAt-a.createdAt);resolve(results)}}})}
